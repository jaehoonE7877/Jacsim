...
| Workflow DI bridge | `Projects/JacsimClient/Sources/UseCaseClients.swift` (external module) |
| Port dependency keys | `Projects/JacsimClient/Sources/*Client.swift` (external module) |
...
- Workflow use case dependency 연결은 `Projects/JacsimClient/Sources/UseCaseClients.swift`에서 관리
- 실제 인프라(`Data` 접근) 의존과 port dependency key 정의는 `Projects/JacsimClient/Sources/*Client.swift` 계층에서만 구체화
...
- `Reducer`에서 Application use case를 직접 인스턴스화 (`Projects/JacsimClient` 경유로 주입)
